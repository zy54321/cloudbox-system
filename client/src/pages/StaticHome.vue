    ...
    <div class="cb-main">
      <div class="cb-grid">
        ...
        <section class="cb-card cb-center-card" style="display:flex;flex-direction:column;min-height:0;">
          ...
          <div class="cb-tools">
            <button class="cb-btn" :class="{ active: activeTool === '起飞' }" type="button"
              @click="onTool('起飞')">起飞</button>
            <button class="cb-btn" :class="{ active: activeTool === '爬升' }" type="button"
              @click="onTool('爬升')">爬升</button>
            <button class="cb-btn" :class="{ active: activeTool === '巡航' }" type="button"
              @click="onTool('巡航')">巡航</button>
            <button class="cb-btn" :class="{ active: activeTool === '进近' }" type="button"
              @click="onTool('进近')">进近</button>
            <button class="cb-btn" :class="{ active: activeTool === '降落' }" type="button"
              @click="onTool('降落')">降落</button>
            <button class="cb-btn ghost" :class="{ active: activeTool === '放大' }" type="button"
              @click="onTool('放大')">放大</button>
            <button class="cb-btn ghost" :class="{ active: activeTool === '缩小' }" type="button"
              @click="onTool('缩小')">缩小</button>
            <button class="cb-btn ghost" :class="{ active: activeTool === '巡航故障' }" type="button"
              @click="onTool('巡航故障')">巡航故障</button>
          </div>

          <div style="flex:1;min-height:0;padding:0 14px 14px;">
            <div class="cb-viewport">
              <div class="cb-overlay-tabs">
                <button class="cb-mini active" type="button">飞机</button>
                <button class="cb-mini" type="button">航迹</button>
                <button class="cb-mini" type="button">卫星</button>
                <button class="cb-mini" type="button">地面</button>
              </div>
              <div class="cb-stage">阶段：降落 ｜ 层级：太空 ｜ 缩放级别：4</div>

              <!-- Cesium视口占位（后续叠加UI overlay） -->
              <CesiumViewport />

              <!-- 右上角飞机信息浮窗 -->
              <div class="cb-float cb-map-popup" :style="{ backgroundImage: `url(${mapPopup})` }">
              ...
              <div v-if="showWarn" class="cb-warn">
                ...
                <div class="cb-warn-bd">巡航阶段遇到故障时弹出或显著提示（脚本/示例驱动）</div>
              </div>
            </div>
          </div>

          <div
            style="padding:0 14px 14px;color:rgba(255,255,255,.75);font-size:12px;display:flex;gap:10px;align-items:center;">
            <span>i</span>
            <span>提示：地图点击（飞机/地面/天空等符号）已在主视图区提供信息框。</span>
          </div>
        </section>
        ...
        <aside class="cb-card cb-panel cb-msg-panel">
          ...
          <div class="cb-panel-tabs">
            ...
            <button class="cb-chip" :class="{ active: activeTab === 'relation' }" type="button"
              @click="activeTab = 'relation'">
              <span class="cb-chip-text">交互关系</span>
            </button>
            <button class="cb-chip" :class="{ active: activeTab === 'modules' }" type="button"
          ...
          <div class="cb-panel-body">
            <template v-if="activeTab === 'base'">
              <div class="cb-item">
                <h4><span style="color:#ffd54a;">星基：</span>高通量卫星、低轨通信卫星</h4>
